...
def plot_value_cvar() -> None:
    ...
    for _, row in data.iterrows():
        ...
        ax.annotate(label, xy=(x, y), xytext=(5, 4), textcoords="offset points", fontsize=7.5, ha="left", va="bottom")
...
def plot_paired_seed_effects() -> None:
    ...
    metrics = [
        ("Value gain", "value_delta_musd", "Value gain\n(M USD)", OKABE_ITO["yellow"]),
        ("CVaR95 loss reduction", "cvar95_loss_reduction_kusd_h", "CVaR95 loss reduction\n(k USD/h)", OKABE_ITO["bluish_green"]),
        ("Imbalance reduction", "imbalance_reduction_gwh", "Imbalance reduction\n(GWh)", OKABE_ITO["sky_blue"]),
    ]
    fig, axes = plt.subplots(1, 3, figsize=(7.3, 3.55), sharey=True)
    for ax, (metric_name, column, xlabel, color) in zip(axes, metrics):
        values = deltas[column].astype(float).to_numpy()
        jitter = np.linspace(-0.23, 0.23, len(values))
        ax.scatter(values, jitter, s=24, color=color, alpha=0.85, edgecolor="none", zorder=3)
        ...
        x_min = min(0.0, float(values.min()), low)
        x_max = max(float(values.max()), high)
        pad = max((x_max - x_min) * 0.14, 0.15)
        ax.set_xlim(x_min - pad, x_max + pad)
        ax.text(0.02, 0.92, f"{int(np.sum(values > 0))}/{len(values)} seeds positive", transform=ax.transAxes, fontsize=8.0, ha="left", va="top")
    fig.tight_layout()
